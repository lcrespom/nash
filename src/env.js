    ...
    setTimeout(async _ => {
        ...
        for (let path of paths) {
            fs.readdir(path, (err, files) =>
                err ? 0 : files.forEach(f => (localCommands[f] = f))
            )
        }
        let out = await runHiddenCommand('alias; echo $?')
        let aliases = commandOut2Array(out)
        for (let alias of aliases) {
            let m = alias.match(/alias (.+)=(.+)/)
            if (!m || !m[1] || !m[2]) continue
            localCommands[alias] = m[1] + ': aliased to ' + m[2]
        }
    }, 100)
}
...
function which(command) {
    if (getUserStatus().isRemote) return whichRemote(command)
    else return whichLocal(command)
}
