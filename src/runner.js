...
const { startPipeServer } = require('./pipe')
...
function dataFromShell(data) {
    switch (state) {
        ...
        case TermState.runningCommand:
            return processCommandOutput(data)
...
function startShell() {
    ...
    process.env.PROMPT_COMMAND = `node src/nash ${process.pid} test`
    ptySetup(shell, params, term, dir)
    startPipeServer(msg => pipeCB(msg))
}
...
async function runCommandInternal(cmd) {
    return new Promise(resolve => {
        theCommand = cmd
        pipeCB = resolve
        state = cmd.length > 0 ? TermState.readingCommand : TermState.runningCommand
...
async function runCommand(line, { pushDir = true } = {}) {
    ...
    if (pushDir) dirHistory.push(ustatus.cwd)
    if (process.cwd() != ustatus.cwdfull) chdirOrWarn(ustatus.cwdfull)
}
