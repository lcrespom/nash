...
function loadNashRCJS() {
    ...
    if (!rcexports.plugins)
        throw new Error(
            'Error: ~/.nash/nashrc.js should export a `plugins` array'
        )
    return rcexports
}
...
function nashStartup() {
    ...
    if (userConfig.options === undefined) userConfig.options = {}
...
function nashShutdown() {
    for (let module of pluginModules) if (module && module.stop) module.stop()
}
